...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    a = 0
    if num_rolls == 0:
        a = boar_brawl(player_score,opponent_score)
    else:
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    a = simple_update(num_rolls,player_score,opponent_score,dice)
    a = sus_points(a)
    return a
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for i in range(10):
        k= i + 1
        t = make_averaged(roll_dice,samples_count)(k,dice)
        if t > score:
            score = t
            times = k
    return times
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    t = score
    a = sus_update(0,score,opponent_score) 
    b = a - t
    if b >= threshold:
        return 0
    return num_rolls # Remove this line once implemented.
